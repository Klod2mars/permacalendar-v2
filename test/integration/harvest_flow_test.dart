  ...
  group('Harvest Flow Integration Test', () {
    ...
    Widget createTestApp(List<Planting> readyPlantings) {
      return ProviderScope(
        overrides: [
          plantingsReadyForHarvestProvider
              .overrideWith((ref) => readyPlantings),
        ],
    ...
    testWidgets('Complete harvest flow: open → select → harvest → confirm',
      ...
      expect(
          find.textContaining('1 plante(s) sélectionnée(s)'), findsOneWidget);

      await tester.tap(find.byType(Checkbox).at(1));
      await tester.pumpAndSettle();

      expect(
          find.textContaining('2 plante(s) sélectionnée(s)'), findsOneWidget);
      ...
      expect(find.textContaining('Voulez-vous récolter 2 plante(s)'),
          findsOneWidget);
    });

    testWidgets('Harvest flow: cancel confirmation',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestApp(mockPlantings));
      await tester.pumpAndSettle();
    ...
    testWidgets('Harvest flow: select all then harvest',
      ...
      expect(
          find.textContaining('3 plante(s) sélectionnée(s)'), findsOneWidget);
      ...
      expect(find.textContaining('Voulez-vous récolter 3 plante(s)'),
          findsOneWidget);
    });
