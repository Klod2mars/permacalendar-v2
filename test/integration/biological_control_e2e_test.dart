    ...
    test('E2E: Complete biological control flow from observation to recommendations', () async {
      ...
      const ladybug = BeneficialInsect(
        ...
        habitat: HabitatRequirements(
          needsWater: true,
          needsShelter: true,
          favorableConditions: ['Fleurs nectarifÃ¨res', 'Zones non traitÃ©es'],
        ),
      ...
      expect(ladybugRec.priority, equals(3)); // Moderate severity â†’ priority 3
      ...
      // âœ… Observation created by USER (not AI)
      expect(pestObservation.id, isNotEmpty);
      expect(pestObservation.observedAt, isNotNull);
      
      // âœ… Recommendations generated by AI (not user)
      for (final rec in recommendations) {
      ...
      // âœ… Unidirectional flow validated
      // Sanctuary (Observation) â†’ Intelligence (Analysis) â†’ Recommendations (AI Output)
