...
void main() {
  ...
  group('Activity Model Tests - Sérialisation et validation complète', () {
    ...
    group('Tests de création et validation', () {
      test('Doit créer une activité avec tous les champs requis', () {
        ...
        expect(activity.description,
            equals('Un nouveau jardin a été créé avec succès'));
    ...
    group('Tests de sérialisation/désérialisation Hive', () {
      test('Doit sauvegarder et récupérer une activité complète dans Hive',
          () async {
        final now = DateTime.now();
        ...
        expect(retrievedActivity.description,
            equals(originalActivity.description));
        expect(retrievedActivity.entityId, equals(originalActivity.entityId));
        expect(
            retrievedActivity.entityType, equals(originalActivity.entityType));
      ...
      test('Doit gérer la sérialisation avec des métadonnées complexes',
          () async {
        ...
        expect(retrieved.metadata['listValue'],
            equals(['item1', 'item2', 'item3']));
        expect(retrieved.metadata['mapValue']['nestedString'],
            equals('nested value'));
        expect(retrieved.metadata['mapValue']['nestedInt'], equals(100));
        expect(retrieved.metadata['nullValue'], isNull);
      });

      test('Doit persister plusieurs activités et les récupérer correctement',
          () async {
    ...
    group('Factory Constructors', () {
      ...
      test('germinationConfirmed factory should create correct activity', () {
        ...
        expect(activity.metadata['germinationDate'],
            equals(germinationDate.toIso8601String()));
    ...
    group('Data Validation', () {
      ...
      test('should validate date consistency', () {
        ...
        expect(
            activity.createdAt.isBefore(activity.updatedAt) ||
                activity.createdAt.isAtSameMomentAs(activity.updatedAt),
            isTrue);
      });
