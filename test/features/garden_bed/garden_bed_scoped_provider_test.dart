...
void main() {
  group('Garden Bed Scoped Provider Unit Tests', () {
    ...
    test('gardenBedProvider should return empty list for non-existent garden',
        () async {
      ...
      final gardenBeds =
          await container.read(gardenBedProvider('non_existent_garden').future);

      expect(gardenBeds, isEmpty);
    });

    test('gardenBedProvider should return correct beds for specific garden',
        () async {
      await GardenBoxes.clearAllGardens();
      ...
      final garden1Beds =
          await container.read(gardenBedProvider(garden1.id).future);
      final garden2Beds =
          await container.read(gardenBedProvider(garden2.id).future);
    ...
    test('gardenBedCountProvider should return correct count', () async {
      ...
      final count =
          await container.read(gardenBedCountProvider(garden.id).future);

      expect(count, equals(3));
    });

    test('gardenTotalAreaProvider should calculate correct total area',
        () async {
      await GardenBoxes.clearAllGardens();
      ...
      // Attendre que les données se chargent et obtenir le résultat
      final totalArea =
          await container.read(gardenTotalAreaProvider(garden.id).future);
    ...
    test('gardenBedDetailProvider should return null for non-existent bed',
        () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);

      // Attendre que les données se chargent et obtenir le résultat
      final foundBed = await container.read(gardenBedDetailProvider(
          (gardenId: 'non_existent_garden', bedId: 'non_existent_bed')).future);
