...
class MockHarvestRecordsNotifier extends HarvestRecordsNotifier {
  ...
  void setRecords(List<HarvestRecord> records) {
    state = HarvestRecordsState(
        records: records, isLoading: false, error: state.error);
  }
}
...
void main() {
  ...
  HarvestRecord createRecord(String id, String plantId, String plantName,
      DateTime date, double totalValue, double kg,
      {String gardenId = 'g1'}) {
  ...
  test('Empty state returns default values', () {
    final params = EconomyQueryParams(
        startDate: DateTime(2025, 1, 1), endDate: DateTime(2025, 12, 31));
  ...
  test('Totals and basic metrics calculation', () {
    final r1 = createRecord(
        '1', 'p1', 'Tomate', DateTime(2025, 6, 15), 100.0, 10.0); // 10€/kg
    final r2 = createRecord(
        '2', 'p2', 'Basilic', DateTime(2025, 6, 20), 50.0, 1.0); // 50€/kg

    (container.read(harvestRecordsProvider.notifier)
            as MockHarvestRecordsNotifier)
        .setRecords([r1, r2]);
  ...
  test('Filtering by date works correctly', () {
    final r1 = createRecord(
        '1', 'p1', 'Tomate', DateTime(2025, 5, 1), 10.0, 1.0); // Before
    final r2 = createRecord(
        '2', 'p1', 'Tomate', DateTime(2025, 6, 1), 20.0, 2.0); // Inside
    final r3 = createRecord(
        '3', 'p1', 'Tomate', DateTime(2025, 8, 1), 30.0, 3.0); // After
    ...
    final params = EconomyQueryParams(
        startDate: DateTime(2025, 5, 15), endDate: DateTime(2025, 7, 15));
    final details = container.read(economyDetailsProvider(params));
  ...
  test('Ranking logic is correct (descending by value)', () {
    final r1 =
        createRecord('1', 'p1', 'LowVal', DateTime(2025, 6, 1), 10.0, 10.0);
    final r2 =
        createRecord('2', 'p2', 'HighVal', DateTime(2025, 6, 1), 100.0, 5.0);
    final r3 =
        createRecord('3', 'p3', 'MidVal', DateTime(2025, 6, 1), 50.0, 5.0);
  ...
  test('Monthly revenue and Top Plant Per Month', () {
    ...
    final r2 = createRecord(
        '2', 'p2', 'P2', DateTime(2025, 6, 20), 30.0, 1.0); // Top in June
    // July: 10
    final r3 = createRecord(
        '3', 'p1', 'P1', DateTime(2025, 7, 5), 10.0, 1.0); // Top in July
  ...
  test('Diversity Index (HHI) logic', () {
    ...
    var details = container.read(economyDetailsProvider(EconomyQueryParams(
        startDate: DateTime(2025, 1, 1), endDate: DateTime(2025, 12, 31))));
    ...
    details = container.read(economyDetailsProvider(EconomyQueryParams(
        startDate: DateTime(2025, 1, 1), endDate: DateTime(2025, 12, 31))));
  ...
  test('Fast vs Long term logic with matching planting', () {
    ...
    (container.read(plantingProvider.notifier) as MockPlantingNotifier)
        .setPlantings([planting]);
    (container.read(harvestRecordsProvider.notifier)
            as MockHarvestRecordsNotifier)
        .setRecords([r1]);
