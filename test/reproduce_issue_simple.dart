...
String _getStringValue(Map<String, dynamic> json, String key, String defaultValue) {
  try {
...
double _getDoubleValue(Map<String, dynamic> json, String key, double defaultValue) {
  try {
    final value = json[key];
...
List<String> _getStringListValue(Map<String, dynamic> json, String key, List<String> defaultValue) {
  try {
    final value = json[key];
    if (value is List) {
...
List<String>? _getOptionalStringListValue(Map<String, dynamic> json, String key) {
  try {
    final value = json[key];
    if (value is List) {
      return value.map((e) => e.toString()).toList();
...
Map<String, dynamic> _getMapValue(Map<String, dynamic> json, String key, Map<String, dynamic> defaultValue) {
  try {
    final value = json[key];
...
Map<String, dynamic>? _getOptionalMapValue(Map<String, dynamic> json, String key) {
  try {
    final value = json[key];
    if (value is Map) return Map<String, dynamic>.from(value);
    return null;
...
void main() {
  final logFile = File('verify_error.txt');
  ...
  log('Starting reproduction script (simple)...');
  try {
    ...
    log('Found ${plantsList.length} plants.');
    ...
    for (var i = 0; i < plantsList.length; i++) {
        ...
        if (plantsList[i] is Map && plantsList[i]['id'] != null) {
          plantId = plantsList[i]['id'].toString();
        }
      ...
      try {
        ...
        error++;
        log('\n!!! FAILURE at plant index $i (ID: $plantId):');
        log('Error type: ${e.runtimeType}');
        log('Error message: $e');
        log('Stacktrace:\n$st');
        // Only show first 1 error to avoid spam
        if (error >= 1) break;
      }
    }
    
    log('Finished: Success=$success, Error=$error');
    if (error > 0) exit(1);
  } catch (e, st) {
    log('Fatal error: $e\n$st');
    exit(1);
