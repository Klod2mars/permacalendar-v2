...
class MigrationOrchestrator {
  ...
  Future<MigrationResult> startCompleteMigration() async {
    ...
    try {
      ...
      _log('âœ… MIGRATION COMPLÃˆTE RÉUSSIE (${duration.inSeconds}s)',
          level: 500);
  ...
  Future<PhaseResult> _executePreparationPhase() async {
    ...
    try {
      ...
      final duration = DateTime.now().difference(startTime);
      _log('âœ… Phase préparation terminée (${duration.inSeconds}s)',
          level: 500);
  ...
  Future<PhaseResult> _executeTransitionPhase() async {
    ...
    try {
      ...
      final duration = DateTime.now().difference(startTime);
      _log('âœ… Phase transition terminée (${duration.inSeconds}s)',
          level: 500);
  ...
  Future<PhaseResult> _executeSwitchPhase() async {
    ...
    try {
      ...
      final duration = DateTime.now().difference(startTime);
      _log('âœ… Phase basculement terminée (${duration.inSeconds}s)',
          level: 500);
  ...
  Future<PhaseResult> _executeCleanupPhase() async {
    ...
    try {
      ...
      if (cleanupResult.success) {
        _log('âœ… Phase nettoyage terminée (${duration.inSeconds}s)',
            level: 500);
      } else {
