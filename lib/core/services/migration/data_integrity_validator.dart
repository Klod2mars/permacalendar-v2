...
class DataIntegrityValidator {
  ...
  Future<bool> validateMigratedGarden(String gardenId) async {
    try {
      ...
      _logError(
          'âŒ Erreur validation post-migration $gardenId', e, stackTrace);
      _issuesDetected++;
      return false;
    }
  }
  ...
  Future<CoherenceResult> validateAllData() async {
    try {
      ...
      if (issues.isNotEmpty) {
        _log('âš ï¸ ${issues.length} problèmes de cohérence détectés',
            level: 900);
      }
  ...
  bool _compareGardens(Garden legacyGarden, GardenFreezed modernGarden) {
    try {
      ...
      if (!descriptionMatch && legacyGarden.description.isNotEmpty) {
        _log('âš ï¸ Descriptions différentes pour ${legacyGarden.id}',
            level: 700);
      }
