...
class MonthRevenue {
  ...
  MonthRevenue(
      {required this.year, required this.month, required this.totalValue});
}
...
final economyDetailsProvider =
  ...
  final filteredRecords = harvestState.records.where((record) {
    if (params.gardenIds.isNotEmpty &&
        !params.gardenIds.contains(record.gardenId)) {
      return false;
    }
  ...
  // For monthly buckets
  final Map<int, List<HarvestRecord>> monthlyBuckets =
      {}; // Key: year*100 + month
  ...
  for (final r in filteredRecords) {
    ...
    final plantKey =
        r.plantId.isNotEmpty ? r.plantId : (r.plantName ?? 'unknown');
  ...
  for (final mKey in sortedMonthKeys) {
    ...
    monthlyRevenueList
        .add(MonthRevenue(year: year, month: month, totalValue: mVal));
    ...
    if (mPlantVal.isNotEmpty) {
      ...
      final bestRecs = recordsInMonth
          .where((r) => (r.plantId == bestEntry.key ||
              (r.plantId.isEmpty && r.plantName == bestEntry.key)))
          .toList();
      String bestName = bestRecs.isNotEmpty
          ? (bestRecs.first.plantName ?? 'Inconnu')
          : 'Inconnu';
      ...
      topPlantMonthMap[mKey] = PlantRanking(
        ...
        percentShare: mVal > 0
            ? (bestEntry.value / mVal) * 100
            : 0.0, // share of the month
      );
    }
  }
  ...
  if (diversityScore > 0.7)
    diversityLabel = 'Très diversifié';
  else if (diversityScore > 0.45)
    diversityLabel = 'Moyen';
  else
    diversityLabel = 'Peu diversifié';
  ...
  if (top3.isNotEmpty) {
    final first = top3[0];
    recText =
        'Vos trois cultures les plus rentables sont ${top3Names.join(", ")}. '
  ...
  for (final pEntry in plantsMap.entries) {
    ...
    for (final r in pRecs) {
      ...
      final validPlantings = plantings
          .where((p) => p.plantId == plantId && p.plantedDate.isBefore(r.date))
          .toList();

      if (validPlantings.isNotEmpty) {
        // Find closest
        validPlantings.sort((a, b) =>
            b.plantedDate.compareTo(a.plantedDate)); // Descending dates
