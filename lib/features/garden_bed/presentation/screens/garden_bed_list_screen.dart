...
class _GardenBedListScreenState extends ConsumerState<GardenBedListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Charger toutes les plantations et le catalogue pour les prédictions
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                ...['Argileux', 'Sableux', 'Limoneux', 'Humifère'].map(
                  (soilType) => Padding(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildStatItem(
            'Moyenne',
            '${stats['averageArea'].toStringAsFixed(1)} m²',
            Icons.analytics,
            theme,
          ),
        ],
    ...
    if (filteredGardenBeds.isEmpty) {
      if (gardenBeds.isEmpty) {
        return EmptyStateWidget(
          ...
          subtitle: 'Commencez par créer votre première parcelle de jardin.',
          actionText: 'Créer une parcelle',
  ...
  void _showCreateGardenBedDialog() async {
    ...
    // Si une parcelle a été créée avec succès, rafraîchir la liste
    if (result == true) {
      await _refreshGardenBeds();
      // Optionnel: afficher un feedback supplémentaire si nécessaire
      // Le SnackBar est déjà affiché par le dialogue
    }
  }
  ...
  void _showEditGardenBedDialog(gardenBed) async {
    ...
    // Si une parcelle a été modifiée avec succès, rafraîchir la liste
    if (result == true) {
      await _refreshGardenBeds();
      // Optionnel: afficher un feedback supplémentaire si nécessaire
      // Le SnackBar est déjà affiché par le dialogue
  ...
  void _showDeleteConfirmation(gardenBed) {
    showDialog(
      ...
      builder: (context) => ConfirmationDialog(
        ...
        content:
            'Êtes-vous sûr de vouloir supprimer "${gardenBed.name}" ?\n\nCette action est irréversible.',
        ...
        onConfirm: () async {
          ...
          if (success && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Parcelle supprimée avec succès')),
            );
          }
        },
      ),
  ...
  void _navigateToGardenBedDetail(gardenBed) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Détail de parcelle à implémenter')),
    );
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Surface totale: ${stats['totalArea'].toStringAsFixed(1)} m²'),
            const SizedBox(height: 8),
            Text(
                'Surface moyenne: ${stats['averageArea'].toStringAsFixed(1)} m²'),
            const SizedBox(height: 16),
            const Text('Répartition par type de sol:'),
            ...stats['soilTypeDistribution'].entries.map(
                  (entry) => Text('• ${entry.key}: ${entry.value}'),
                ),
            const SizedBox(height: 8),
            const Text('Répartition par exposition:'),
