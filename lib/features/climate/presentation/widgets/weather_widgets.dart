...
class WindCard extends StatelessWidget {
          ...
          children: [
            Row(
              children: [
                ...
                Text('VENT', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Colors.blueGrey)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
              ...
              children: [
                _buildInfo(context, 'Vitesse', '${windSpeed.toStringAsFixed(1)} km/h'),
                _buildDirectionArrow(windDirection),
                _buildInfo(context, 'Rafales', '${windGusts.toStringAsFixed(1)} km/h', isWarning: windGusts > 40),
              ],
            ),
                        ...
                        itemBuilder: (ctx, i) {
                            ...
                            return Padding(
                                ...
                                child: Column(
                                    ...
                                    children: [
                                        Text('${p.time.hour}h', style: const TextStyle(fontSize: 10)),
                                        const SizedBox(height: 4),
                                        Transform.rotate(
                                        ...
                                        const SizedBox(height: 4),
                                        Text('${p.windSpeedkmh.round()}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                    ],
                                ),
                            );
                        },
  ...
  Widget _buildInfo(BuildContext context, String label, String value, {bool isWarning = false}) {
    return Column(
      children: [
  ...
  Widget _buildDirectionArrow(int degrees) {
    ...
    return Column(
      children: [
        Transform.rotate(
          angle: (degrees - 180) * (math.pi / 180), // Icon points up by default usually, navigation icon points up
          child: const Icon(Icons.navigation, size: 32, color: Colors.blue),
        ),
        Text('$degrees° $cardDir', style: const TextStyle(fontWeight: FontWeight.bold)),
      ],
    );
  }
}
...
class PrecipitationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final future = hourly.where((p) => p.time.isAfter(now.subtract(const Duration(hours: 1))) && p.time.isBefore(now.add(const Duration(hours: 24)))).toList();
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('PRÉCIPITATIONS (24h)', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Colors.blue)),
              ],
            ),
            const SizedBox(height: 16),
            SizedBox(
                    ...
                    itemBuilder: (ctx, i) {
                        ...
                        return Container(
                            ...
                            decoration: BoxDecoration(
                                color: isRainy ? Colors.blue.withOpacity(0.1 + (math.min(p.precipitationMm, 5.0) / 10)) : Colors.transparent,
                                borderRadius: BorderRadius.circular(8),
                                border: isRainy ? Border.all(color: Colors.blue.withOpacity(0.3)) : null,
                            ),
                            child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                    Text('${p.time.hour}h', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                                    const Spacer(),
                                    if (p.precipitationProbability > 0)
                                        Text('${p.precipitationProbability}%', style: const TextStyle(fontSize: 9, color: Colors.blueAccent)),
                                    const SizedBox(height: 4),
                                    if (p.precipitationMm > 0)
                                        Container(
                                            height: math.max(4.0, math.min(30.0, p.precipitationMm * 5)), // Visual bar
                                            width: 8,
                                            decoration: BoxDecoration(
                                                color: Colors.blue,
                                    ...
                                    const SizedBox(height: 4),
                                    Text('${p.precipitationMm}', style: TextStyle(fontSize: 12, fontWeight: isRainy ? FontWeight.bold : FontWeight.normal)),
                                    const Spacer(),
                                ],
                            ),
                        );
...
class SunMoonCard extends StatelessWidget {
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('ASTRES', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: Colors.deepOrange)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                    _buildAstreColumn('Soleil', Icons.wb_sunny, sunrise ?? '--:--', sunset ?? '--:--', Colors.orange),
                    Container(width: 1, height: 40, color: Colors.grey.withOpacity(0.3)),
                    _buildAstreColumn('Lune', Icons.nightlight_round, moonrise ?? '--:--', moonset ?? '--:--', Colors.indigo),
                ],
            ),
            if (moonPhase != null) ...[
                const SizedBox(height: 12),
                Text(_getMoonPhaseName(moonPhase!), style: const TextStyle(fontStyle: FontStyle.italic, fontSize: 12, color: Colors.indigo)),
            ]
          ],
        ),
      ),
  ...
  Widget _buildAstreColumn(String title, IconData icon, String up, String down, Color color) {
    return Column(
        children: [
              ...
              children: [
                   Row(
                    children: [
                        ...
                        const SizedBox(width: 8),
                        Text('RÉSUMÉ JOUR', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold, color: Colors.grey)),
                    ],
                   ),
                   const SizedBox(height: 12),
                   Row(
                       ...
                       children: [
                           _buildItem('Max', '${day.tMaxC?.toStringAsFixed(1) ?? '--'}°'),
                           _buildItem('Min', '${day.tMinC?.toStringAsFixed(1) ?? '--'}°'),
                           _buildItem('Pluie', '${day.precipMm}mm'),
                           _buildItem('Vent Max', '${day.windSpeedMax?.round() ?? '--'}k'),
                       ],
                   )
              ],
          ),
