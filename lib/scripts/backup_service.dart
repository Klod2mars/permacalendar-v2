...
class BackupService {
  static Future<void> exportAll() async {
    ...
    try {
      // 1. Export Activities
      final activities =
          GardenBoxes.activities.values.map((e) => e.toJson()).toList();
      final activitiesFile =
          File('${backupDir.path}/activities_backup_$dateStr.json');
      ...
      // 2. Export Plantings
      final plantings =
          GardenBoxes.plantings.values.map((e) => e.toJson()).toList();
      final plantingsFile =
          File('${backupDir.path}/plantings_backup_$dateStr.json');
