...
class SkyCalibrationOverlay extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SkyCalibrationOverlay> createState() => _SkyCalibrationOverlayState();
}
...
class _SkyCalibrationOverlayState extends ConsumerState<SkyCalibrationOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final topHandle = toScreen(0, -ryPixels - 40); // Rotation handle slightly above
        ...
        children: [
          ...
          CustomPaint(
            painter: _CalibrationGuidePainter(
               cx: cxPixels, cy: cyPixels, 
               rx: rxPixels, ry: ryPixels, 
          ...
          Positioned(
            ...
            child: GestureDetector(
               onPanUpdate: (details) {
                   ...
                   final sinR = math.sin(config.rotation);
                   // Local delta X
                   final localDx = (details.delta.dx * cosR + details.delta.dy * sinR);
                   notifier.updatePartial(rx: (config.rx + localDx / w).clamp(0.01, 1.0));
               },
          ...
          Positioned(
            ...
            child: GestureDetector(
               onPanUpdate: (details) {
                   ...
                   final localDy = (-details.delta.dx * sinR + details.delta.dy * cosR);
                   notifier.updatePartial(ry: (config.ry + localDy / h).clamp(0.01, 1.0));
               },
          ...
          Positioned(
            ...
            child: GestureDetector(
               onPanUpdate: (details) {
                   ...
                   notifier.updatePartial(rotation: config.rotation + details.delta.dx * 0.01);
               },
               child: _Handle(icon: Icons.rotate_right, color: Colors.orangeAccent),
            ),
          ),
          ...
          const Positioned(
             ...
             child: Text(
               ...
               style: TextStyle(color: Colors.white, shadows: [Shadow(blurRadius: 2, color: Colors.black)]),
             ),
          )
        ],
      );
...
class _CalibrationGuidePainter extends CustomPainter {
  ...
  _CalibrationGuidePainter({required this.cx, required this.cy, required this.rx, required this.ry, required this.rotation});
