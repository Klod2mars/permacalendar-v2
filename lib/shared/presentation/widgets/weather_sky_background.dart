  ...
  Widget build(BuildContext context) {
    ...
    return weatherAsync.when(
      data: (weather) {
        ...
        DailyWeatherPoint? findDailyForProjectedDate(
            List<DailyWeatherPoint> daily, DateTime projected) {
          final pDate =
        ...
        final daily =
            findDailyForProjectedDate(weather.dailyWeather, projectedTime);
        ...
        final bool isSunriseValid =
            sunrise != projectedTime && sunrise.year > 2000;
        final bool isSunsetValid =
            sunset != projectedTime && sunset.year > 2000;

        final safeSunrise = isSunriseValid
            ? sunrise
            : DateTime.utc(projectedTime.year, projectedTime.month,
                projectedTime.day, 6, 0);

        final safeSunset = isSunsetValid
            ? sunset
            : DateTime.utc(projectedTime.year, projectedTime.month,
                projectedTime.day, 20, 0);

        // On calcule 'progress' (0.0 Sunrise -> 1.0 Sunset) pour la position du soleil
        final dayProgress =
            _calculateDayProgress(projectedTime, safeSunrise, safeSunset);
...
class _OrganicSkyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (elevation > -0.05) {
      ...
      final paint = Paint()
        ...
        ..blendMode = BlendMode
            .srcOver; // On utilise srcOver ou un mode très léger pour ne pas "brûler" le fond
      ...
      if (darkness > 0.01) {
        ...
        if (darkness < 0.4) {
          ...
          nightColor = Color.lerp(Colors.transparent, const Color(0xFF0F172A),
              t)!; // Bleu nuit plus riche
        } else {
          final t = ((darkness - 0.4) / 0.6).clamp(0.0, 1.0);
          nightColor = Color.lerp(const Color(0xFF0F172A),
              const Color(0xFF020205), t)!; // Noir profond
        }

        final paint = Paint()
          ..color = nightColor
          ..maskFilter = const MaskFilter.blur(
              BlurStyle.normal, 15) // Flou légèrement réduit
  ...
  void _drawLiveStars(Canvas canvas, Rect rect, double opacity) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if ((x * x) / ((rect.width / 2) * (rect.width / 2)) +
