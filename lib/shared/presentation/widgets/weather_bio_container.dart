...
class WeatherBioContainer extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<WeatherBioContainer> createState() =>
      _WeatherBioContainerState();
}
...
class _WeatherBioContainerState extends ConsumerState<WeatherBioContainer>
  ...
  void _onHorizontalDragUpdate(DragUpdateDetails details) {
    ...
    setState(() {
      ...
      if (_dragOffsetPixels > 12 * _pixelsPerHour)
        _dragOffsetPixels = 12 * _pixelsPerHour;

      // Update Shared State for Sky
      ref
          .read(weatherTimeOffsetProvider.notifier)
          .setOffset(_dragOffsetPixels / _pixelsPerHour);
    });
  }
  ...
  HourlyWeatherPoint? _findNearest(
      List<HourlyWeatherPoint> hourly, DateTime target) {
  ...
  Widget build(BuildContext context) {
    ...
    final projectedTime = DateTime.now()
        .toUtc()
        .add(Duration(minutes: (hoursOffset * 60).round()));
        ...
        children: [
          if (isTimeTraveling)
            weatherAsync.when(
              data: (data) {
                final point =
                    _findNearest(data.result.hourlyWeather, projectedTime);
                final temp =
                    point?.temperatureC ?? data.result.currentTemperatureC ?? 0;
